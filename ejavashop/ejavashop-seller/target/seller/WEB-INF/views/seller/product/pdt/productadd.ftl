        ...
        var options = {
            ...
            success: function (data) {
                if (data && null != data.success && data.success == true) {
                    alert("商品发布成功");
                    window.location.href='${domainUrlUtil.EJS_URL_RESOURCES}/seller/product/chooseCate';
...
<div class="wrapper">
    <div class="formbox-a">
        <h2 class="h2-title">发布商品-填写商品详细信息
            <span class="s-poar">
                <a class="a-back" href="${domainUrlUtil.EJS_URL_RESOURCES}/seller/product/chooseCate">返回</a>
            </span>
        </h2>
                        ...
                        <p class="p12 p-item">
                            ...
                            <#if sellerCate?? && sellerCate?size&gt; 0>
                                <select id="sellerCate_0" name="sellerCate_0"
                                        missingMessage="请选择本店分类"
                                        data-options="required:true"
                                        level="0" class="w210" <#if product??>style="display:none"</#if>>
